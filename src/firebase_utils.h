...
void init_wifi(){
    ...
    do{
        log2ser("WiFi-Status: ", WiFi.status(), " / Connecting to Wi-fi...");
        delay(2000);
...
uint16_t upload_fb(float humidity, float temperature, String root="/"){
    ...
    if( !Firebase.RTDB.pushFloat(&firebaseData, path, humidity) ){
        log2ser("Failed to push humidity value!");
        ++failed;
    }

    path  = root;    path.concat("temp");
    if( !Firebase.RTDB.pushFloat(&firebaseData, path, temperature) ){
